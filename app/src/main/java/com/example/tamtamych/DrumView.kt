...
class DrumView(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {

    private var adjustedAngle: Float = 0f
    private var colorAtTwelveName: String = "Неизвестный цвет"
    ...
    init {
        ...
        val colorNames = mapOf(
            0xFFFF0000.toInt() to "Красный",
            0xFFFFA500.toInt() to "Оранжевый",
            0xFFFFFF00.toInt() to "Желтый",
            0xFF008000.toInt() to "Зеленый",
            0xFF00FFFF.toInt() to "Голубой",
            0xFF0000FF.toInt() to "Синий",
            0xFF800080.toInt() to "Фиолетовый"
        )

        for (i in sectorAngles.indices) {
            val (startAngle, endAngle) = sectorAngles[i]
            if (adjustedAngle in startAngle..endAngle) {
                colorAtTwelveName = colorNames[sectorColors[i]] ?: "Неизвестный цвет"
                break
            }
        }
    }
